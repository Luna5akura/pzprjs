...
(function(pidlist, classbase) {
	...
	Graphic: {
		...
		getNumberVerticalOffset: function(cell) {
			return this.getCellVerticalOffset(cell);
		},
		...
		drawCells_common: function(header, colorfunc) {
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (!!color) {
					...
					this.drawDiamondCell(g, px, py, rw, rh, true);
				} else {
					g.vhide();
				}
			}
		},
		drawDiamondCell: function(g, px, py, rw, rh, close) {
			g.beginPath();
			g.moveTo(px - rw, py);
			g.lineTo(px, py - rh);
			g.lineTo(px + rw, py);
		...
		drawSolverOverlayLines: function() {
			...
			for (var i = 0; i < blist.length; i++) {
				...
				if (entry && !this.hasGravelAnswerBorderState(border)) {
					...
					this.drawGravelSolverBorderLine(g, border, lm);
		...
		drawGravelSolverBorderLine: function(g, border, lm) {
		...
		drawSolverOverlayPekes: function() {
			...
			for (var i = 0; i < blist.length; i++) {
				...
				if (entry && !this.hasGravelAnswerBorderState(border)) {
					...
					g.strokeCross(
						(border.tsx + border.tex) * (this.bw / 2),
						(border.tsy + border.tey) * (this.bh / 2),
						size - 1
					);
				} else {
					g.vhide();
